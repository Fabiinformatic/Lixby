...
foreach ($langDir in $langDirs) {
  ...
  foreach ($aboutPath in @((Join-Path $langDir.FullName 'about.html'), (Join-Path $langDir.FullName 'pages\\about.html'))) {
    ...
    $about = $about -replace '(?s)<header class="nav"[\s\S]*?</header>\s*<div class="nav-mobile-panel"[\s\S]*?</div>', $headerAdj
    $about = $about -replace '(?s)<footer class="footer[\s\S]*?</footer>', $footerAdj
